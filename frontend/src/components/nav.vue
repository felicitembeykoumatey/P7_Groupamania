...
export default {
  ...
  beforeMount() {
    ...
    axios
      .get(`http://localhost:3000/api/param/${this.user_id}`, {
        headers: {
  ...
  <div>
    <nav>
      <div>
        <!-- User Img -->
        <router-link
          :to="{ name: 'User', params: { id: user_id } }"
          class="user_img"
        >
          <img v-if="img" :src="img" title="Tableau de bord" />
          <img
            v-else-if="sex === 'M'"
            src="../assets/user_male.svg"
            id="no_image_user"
            title="Tableau de bord"
          />
          <img
            v-else
            src="../assets/user_female.svg"
            id="no_image_user"
            title="Tableau de bord"
          />
        </router-link>
        ...
        <!-- Params -->
        <router-link :to="{ name: 'Parametre', params: { id: user_id } }"
          ><i class="fas fa-tools" title="Paramètres"></i
        ></router-link>
      </div>

      <!-- Activity -->
      <router-link :to="{ name: 'Activity' }"
        ><i class="fas fa-globe" title="Activité"></i
      ></router-link>

      <!-- All Users only for admin -->
      <router-link to="/User" v-if="role === 2"
        ><i class="fas fa-users" title="Tous les utilisateurs"></i
      ></router-link>
    </nav>
    <div id="container">
      <!-- Header -->
