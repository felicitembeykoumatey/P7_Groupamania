...
export default new Vuex.Store({
  ...
  actions: {
    ...
    userPostSignup(context) {
      ...
      if (
        context.state.login_signup.passwordone ===
        context.state.login_signup.passwordtwo
      ) {
        ...
        if (username_valid && email_valid) {
          axios
            ...
            .catch((error) => {
              alert(
                `Quelque chose c'est mal passé. Essayez à nouveau et vérifiez que la sécurité du mot de passe ne soit pas faible. ${error}`
              );
            });
        } else {
          alert(
            "Le formulaire n'est pas valide. Les nombres sont interdis pour le username. Vérifer que l'adresse mail est valide"
          );
        }
      } else {
    ...
    userPostLogin(context) {
      ...
      if (email_valid) {
        ...
        alert(
          "Le formulaire n'est pas valide. Vérifer que l'adresse mail est valide"
        );
      }
    },
    userSignout() {
    ...
    postCommentCreate(context) {
      ...
      if (content_valid) {
        ...
        axios
          .post(`http://localhost:3000/api/comment/${post_id}`, comment, {
            headers: {
    ...
    putCommentEdit(context) {
      ...
      if (content_valid) {
        ...
        axios
          .put(`http://localhost:3000/api/comment/${comment_id}`, comment, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Token ${userStorage.token}`,
            },
        ...
        alert(
          "Le contenu du post n'est pas valide ou est inexistant. Si aucune modification a été faite appuyer sur Annuler"
        );
      }
    },
    deleteCommentDelete(context) {
      ...
      //Send info to Back
      axios
        .delete(`http://localhost:3000/api/comment/${comment_id}`, {
          headers: {
            Authorization: `Token ${userStorage.token}`,
          },
        })
    ...
    putParamEdit(context) {
      ...
      if (username_valid && email_valid) {
        ...
        // Données du formulaire d'initialisation
        ...
        axios
          .put(`http://localhost:3000/api/param/user/${user_id}`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
              Authorization: `Token ${userStorage.token}`,
            },
    ...
    putPassWordEdit(context) {
      ...
      if (
        context.state.param.newpasswordone ===
        context.state.param.newpasswordtwo
      ) {
        //Données d'initialisation
        ...
        axios
          .put(
            `http://localhost:3000/api/param/password/${user_id}`,
            PWchange,
            {
              headers: {
                "Content-Type": "application/json",
          ...
          .catch((error) => {
            alert(`Oups! Quelque chose c'est mal passé.${error}`);
          });
      } else {
        alert("Les mots de passe ne sont pas identiques");
      }
    ...
    deleteUserDelete(context) {
      ...
      //Retour
      axios
        .delete(`http://localhost:3000/api/param/${user_id}`, {
          headers: {
            Authorization: `Token ${userStorage.token}`,
          },
        })
        .then(() => {
          //Si l'utilisateur sipprimé est celui qui était actif = déconnexion
          if (user_id === userStorage.id) {
            sessionStorage.removeItem("userToken");
            window.location.href = "http://localhost:8080/";
          }
          //Sinon, allez à AllUser
          else {
            window.location.href = "http://localhost:8080/User";
          }
        })
        .catch((error) => {
          alert(`oups! erreur.${error}`);
        });
    },
  },
  modules: {},
